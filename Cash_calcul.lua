...
-- ===============================================
-- 🏆 PRESTIGE BOT ISOLÉ (AUTONOME)
-- ===============================================

local PrestigeBot = {}
PrestigeBot.Enabled = false
PrestigeBot.Thread = nil

function PrestigeBot:Start()
    if self.Enabled then return end
    self.Enabled = true

    self.Thread = task.spawn(function()

        -- ===== COPIE STRICTE DE TON CODE =====
        local Players = game:GetService("Players")
        ...
        local TARGET = {
            Stand1=true, Stand2=true, Stand3=true, Stand4=true,
            Stand5=true, Stand6=true, Stand7=true, Stand8=true
        }
        ...
        local function upgradeAllSafe(stands)
            for _, stand in ipairs(stands) do
                if not PrestigeBot.Enabled then return end
                ...
                if level and level > 0 and level < 50 then
                    if (levelCache[stand.Name] or 0) >= 50 then continue end
                    pcall(function()
        ...
        -- ===== LOOP AUTONOME =====
        print("🏆 Prestige Bot lancé")

        while PrestigeBot.Enabled do
            ...
            upgradeAllSafe(standsArray)
            handlePrestigeAndSwap(standsArray)
        end

        print("🛑 Prestige Bot stoppé")
    end)
end

function PrestigeBot:Stop()
    self.Enabled = false
end
